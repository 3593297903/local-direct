...
test("sanitizes internal render labels and asset ids into Chinese user-facing text", () => {
  const text = [
    ...
    "props: chat-log, qq_records, digital-records, video_prompt_segment",
  ].join("\n");

  const sanitized = sanitizeInternalPromptTokens(text);

  assert.doesNotMatch(sanitized, /single-segment|AnalysisResult|forensic_room|case-room|chat-log|qq_records|digital-records|video_prompt_segment/i);
  assert.match(sanitized, /单段视频提示词结果/);
  assert.match(sanitized, /法医室/);
  assert.match(sanitized, /专案会议室/);
  assert.match(sanitized, /聊天记录证据/);
  assert.match(sanitized, /QQ聊天记录/);
...
test("deep sanitizer cleans nested prompt result objects without changing structure", () => {
  const value = {
    contentType: "single-segment AnalysisResult",
    workflow: {
      fullVideoPrompt: "chat-log 在 digital-records 中被打开，qq_records 被贴到墙上。",
    },
    storyboard: [
      {
        scene: "case-room",
  ...
  assert.equal(sanitized.workflow.fullVideoPrompt, "聊天记录证据 在 数字证据 中被打开，QQ聊天记录 被贴到墙上。");
...
test("detects internal tokens before saving user-facing prompt text", () => {
  ...
  assert.equal(findInternalPromptToken("qq_records")?.token, "qq_records");
